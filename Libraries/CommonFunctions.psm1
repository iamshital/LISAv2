...
function Install-CustomLIS ($CustomLIS, $allVMData, [switch]$RestartAfterUpgrade, $TestProvider)
{
	try
	{
		...
		{
			Write-LogErr "Only lisnext and *.tar.gz links are supported. Use -CustomLIS lisnext/<optional_branch_name>. Or use -CustomLIS <link to tar.gz file>"
		}
		else
		{
	...
	if ($CustomLIS) {
		...
		if(@("REDHAT", "ORACLELINUX", "CENTOS").contains($global:detectedDistro)) {
			...
			$LISUpgradeStatus = Install-CustomLIS -CustomLIS $CustomLIS -allVMData $AllVMData `
				-RestartAfterUpgrade -TestProvider $TestProvider
			if (!$LISUpgradeStatus) {
