...
$deployVMS = DeployVMS -setupType $currentTestData.setupType `
                        -Distro $Distro -xmlConfig $xmlConfig

if ($deployVMS) {
    try {
        # Prepare / Upload all the required files to guest VM.
        RemoteCopy -uploadTo $AllVMData.PublicIP -port $AllVMData.SSHPort `
        ...
        $out = RunLinuxCmd -username $user -password $password -ip $AllVMData.PublicIP `
            -port $AllVMData.SSHPort -command "chmod +x *" -runAsSudo

        # Execute the script.
        LogMsg "Executing : $($currentTestData.testScript)"
        RunLinuxCmd -username $user -password $password -ip $AllVMData.PublicIP `
            -port $AllVMData.SSHPort -command "bash -c ./$($currentTestData.testScript)" `
            -runAsSudo -runMaxAllowedTime 7200 -maxRetryCount 0
        RemoteCopy -download -downloadFrom $AllVMData.PublicIP `
            -files "/home/$user/TestState.log, /home/$user/TestExecution.log, /home/$user/TestExecutionError.log" `
